...
# 선택 1 - 토너먼트 선택
# 현재 개체군에서 무작위로 K개의 개체를 선택 후
# 가장 적합도(fitness)가 높은 개체를 선택하여 반환
# 개체수가 채워질 때 까지 반복
# K가 클수록 적합도가 높은 개체가 선택될 확률이 크다.
def select_tmt(population, popRanked):

    # population 의 10%를 k로 설정하여 후보자 리스트 만듦
    # 만약 100개면 20개를 뽑아서 토너먼트 진행
    k = int(len(popRanked) * 0.1)
...
def geneticAlgorithmPlot(population, popSize, mutationRate, generations):
    ...
    plt.plot(progress)
    plt.title('Tournament with Cycle crossover')
    plt.ylabel('Distance')
